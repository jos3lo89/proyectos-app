...
const UpdateProgress = ({ progress }: UpdateProgressProps) => {
  ...
  const form = useForm<AddProgressType>({
    ...
    defaultValues: {
      ...
      progressDate: progress.progressDate
        ? formatToInputDate(progress.progressDate)
        : getTodayForInput(),
    },
  });
  ...
  const onSubmit = (values: AddProgressType) => {
    startTransition(async () => {
      const res = await updateProgress(progress.id, values);
      if (res.error) {
        toast.error("Error", {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <SquarePen className="mr-2 h-5 w-5 cursor-pointer text-amber-500" />
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Actulizar el Avance</DialogTitle>
